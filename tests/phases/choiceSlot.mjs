/**
 * choice beat 的 `slot` 分流（§22.2，S39 定義／S43 改義）。
 *
 * `slot` 標的是**這個 choice 屬於哪一類**：卡牌內的即時反應（月度事件卡／扮演卡）
 * 是 `'inline'`；規劃型決策（訓練菜單／備賽戰術／轉會退役）是 `'act'`。
 * slot 由發射端指定，不由 UI 猜——猜錯的那天沒有測試會紅，所以這裡把兩件事釘死：
 *
 *   1. 每個發射點都**顯式**宣告 slot，UI 永遠不必兜底
 *   2. 1–3 類走 `'inline'`（且前一個 beat 必是 card——UI 要把問答狀態標到那張卡上），
 *      第 4 條（賽事五拍第 2 拍）與規劃型決策一律 `'act'`
 *
 * ⚠ v4.6.4（S43）起兩類**都畫在底部決策槽**（單一操作點，§22.2）。`'inline'` 不再
 * 代表「畫在卡片下緣」，只代表「有一張發問的錨點卡」——UI 據此在卡上掛等待提示與
 * 選後定格文字。引擎側的協定完全沒變，所以本檔的斷言一字未動。
 */
