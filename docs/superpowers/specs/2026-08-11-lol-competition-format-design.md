...
原本種子決定一切——同一個種子只會有一段人生，重玩沒有意義。但種子該代表的是「你生
下來是什麼樣的人」，不是「你會過什麼樣的人生」。每個人都該有自己的人生。

拆成兩條亂數流：

| 流 | 種子來源 | 決定什麼 |
|---|---|---|
| **出生** | 玩家輸入或分享的種子字串 | 起始能力、潛力天花板、性格底色、最早會的三隻英雄、一開始混哪個網咖隊 |
| **人生** | 每次開新局隨機抽 | 事件卡、訓練骰、勝負、傷病、報價、扮演後的外界反應 |

引擎本身仍是 `(出生種子, 人生種子, 選擇)` 的確定性函式——**隨機的是「誰來決定人生
種子」，不是引擎**。這一點很重要：存檔續玩要接回同一段人生，回歸測試要能比對，兩者
都需要引擎可重現。遊戲每次開新局隨機抽人生種子，測試則明確指定。

因此黃金種子快照仍然是逐位元比對，不必退化成統計檢查。

存檔要同時存出生種子（在 `state.seed`）與人生種子（`lifeSeed`）＋人生流的進度，
否則續玩會變成「拿同樣的天賦重開一段新人生」。`SAVE_VERSION` 升到 6。

**順帶修掉的 BUG**：`createState` 的回傳物件把 `seed` 宣告了兩次——第一次是種子
字串，第二次是世界賽種子序（`seed: 0`）。後者覆蓋前者，所以出生種子從來沒有真的
存進 state，`state.seed` 全域指的其實是種子序。種子序改名 `seedRank`。

分享連結（`?seed=`）的語意也跟著變成「同天賦、新人生」，結算頁的按鈕文案一併改成
「複製天賦連結」與「同天賦再走一次」。
