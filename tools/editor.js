...
function renderTutorial() {
  ...
  wrap.append(h(2, '新手教學：從零開始建立事件和特質'));
  wrap.append(p('這個編輯器是「電競人生」遊戲的內容製作工具。照著表單填資料就能設計遊戲內容，不需要寫程式。'));

  /* ---- 基本概念 ---- */
  wrap.append(h(3, '先搞懂基本概念'));
  wrap.append(p('<strong>事件卡</strong>：玩家每月會遇到的互動選擇。選完後根據結果改變角色屬性。'));
  wrap.append(p('<strong>特質卡</strong>：角色的被動能力，永久持有。來源有兩種：事件旗標一次觸發（common），或滿足條件後年度結算自動授予（unique）。'));
  wrap.append(p('<strong>屬性</strong>：六項基礎能力——體能（vit）、靈巧（agi）、意識（awr）、技巧（tec）、默契（syn）、決斷（dec）。只有事件結果能改屬性，特質不能。'));
  wrap.append(p('<strong>事件計數</strong>：每張事件卡引擎自動計算出場次數。你可以寫條件「這張卡出過 N 次才…」。'));
  wrap.append(p('<strong>事件旗標</strong>：事件可以點亮或熄滅一個記號，讓另一張事件讀到。這是做連續事件鏈的基礎。'));
  wrap.append(p('<strong>效果鍵</strong>：寫在特質上的加成，改的是遊戲機制（受傷機率、心理值等），不是屬性。'));

  wrap.append(h(3, '我們要做什麼'));
  wrap.append(p('<strong>事件卡</strong>「鍛鍊體能」（id: <code>squat_challenge</code>）——去健身房遇到「館長」要求深蹲。成功加體能和決斷，失敗扣動機和體能。'));
  wrap.append(p('<strong>特質</strong>「深蹲救臺灣」（unique）——事件卡出場 7 次後，年度結算自動授予。效果是降低受傷風險、加抗壓，副作用是扣紀律。'));
  wrap.append(note('本教學的示範資料已實際寫入遊戲資料檔，可以在「事件卡」分頁搜尋「鍛鍊體能」查看。'));

  /* ---- 步驟 1 ---- */
  wrap.append(step(1, '新增事件卡'));
  wrap.append(p('點「事件卡」分頁 →「+ 新增」→ 左邊列表會出現 <code>new_event</code>，點它。'));

  wrap.append(h(4, '基本欄位'));
  wrap.append(field('id', '英文代號，不可重複。改 <code>squat_challenge</code>。'));
  wrap.append(field('名稱', '玩家看到的名字。填「鍛鍊體能」。'));
  wrap.append(field('種類', '選「一般」。'));
  wrap.append(field('分類池', '勾 persona（人格類事件）。'));
  wrap.append(field('子標籤', '選「生活」。'));
  wrap.append(field('時段標籤', '勾「常規賽期間」——只有賽季中出現。'));
  wrap.append(field('互斥群組', '填 <code>solo_squat_challenge</code>。<code>solo_</code> 表示不與別卡互斥。'));
  wrap.append(warn('互斥群組必填。'));

  /* ---- 步驟 2 ---- */
  wrap.append(step(2, '故事文字與選項'));
  wrap.append(field('敘事文本', '填：'));
  wrap.append(code('你去健身房遇到館長，他指著深蹲架說：「來，做五組，每組 10 下，重量我決定。」\n你要接受挑戰嗎？'));

  wrap.append(h(4, '選項設定'));
  wrap.append(p('至少 2 個、最多 4 個。「成功基準%」是起始機率（實際會被體力、抗壓微調）。'));
  wrap.append(p('<strong>選項 A</strong>（accept，主推）：'));
  wrap.append(field('選項文字', '「接受館長的挑戰」'));
  wrap.append(field('成功基準 %', '55'));
  wrap.append(field('成功倍率／失敗倍率', '都填 1'));
  wrap.append(p('<strong>選項 B</strong>（decline）：'));
  wrap.append(field('選項文字', '「謝謝館長，我今天做別的」'));
  wrap.append(field('成功基準 %', '100（婉拒不會失敗）'));
  wrap.append(field('成功倍率／失敗倍率', '都填 0（沒有任何效果）'));

  /* ---- 步驟 3 ---- */
  wrap.append(step(3, '成功與失敗結果'));
  wrap.append(p('事件成功走「好結果」，失敗走「壞結果」。'));
  wrap.append(h(4, '好結果'));
  wrap.append(field('結果敘事', '「你完成了五組深蹲，館長點頭說不錯。」'));
  wrap.append(field('體能（vit）', '+2'));
  wrap.append(field('決斷（dec）', '+1'));
  wrap.append(h(4, '壞結果'));
  wrap.append(field('結果敘事', '「第三組就沒力了，館長搖頭走開。」'));
  wrap.append(field('體能（vit）', '−1'));
  wrap.append(field('動機（drive）', '−2（這是心理維度，放在結果的 mental 欄位）'));
  wrap.append(note('六屬性只有 vit／agi／awr／tec／syn／dec。心理維度（動機、自信等）寫在結果的 mental 欄位。'));

  /* ---- 步驟 4 ---- */
  wrap.append(step(4, '建立獨有特質'));
  wrap.append(p('「深蹲救臺灣」是一個 <strong>unique（獨有）</strong>特質——它不進合成樹，由生涯條件直接授予。點「特質卡」分頁，在資料檔最下方的 UNIQUE_TRAITS 區域建立。'));

  wrap.append(field('特質鍵', '<code>squat_savior</code>'));
  wrap.append(field('名稱', '「深蹲救臺灣」'));
  wrap.append(field('種類', 'unique（獨有）'));
  wrap.append(field('池歸屬', 'career（生涯層）'));
  wrap.append(field('描述', '「底盤穩了，受傷機率降低，但太專注深蹲忘了看小地圖。」'));

  wrap.append(h(4, '益處（效果鍵）'));
  wrap.append(field('injuryRate', '寫法：封頂，值 0.7——受傷率不會超過原本的 70%'));
  wrap.append(field('mental_comp', '寫法：加法，值 +3——抗壓值 +3'));

  wrap.append(h(4, '副作用（效果鍵）'));
  wrap.append(field('mental_disc', '寫法：加法，值 −3——紀律 −3（太沉迷重訓忽略了遊戲練習）'));
  wrap.append(field('副作用分級', '選「輕度」'));

  wrap.append(h(4, '授予條件（grantWhen）'));
  wrap.append(p('這是獨有特質的核心——寫一個條件，當條件成立時遊戲年度結算會自動授予特質。'));
  wrap.append(p('填條件式：'));
  wrap.append(code(`['eventCount', 'squat_challenge', 'gte', 7]`));
  wrap.append(p('意思是：「鍛鍊體能」這張卡出場次數 ≥ 7 次時，授予「深蹲救臺灣」。'));
  wrap.append(note('引擎自動以事件卡 id 為鍵計算出場次數。你不需宣告任何計數器。'));

  /* ---- 步驟 5 ---- */
  wrap.append(step(5, '進階：事件旗標與連續事件'));
  wrap.append(p('如果你想做更複雜的事件鏈（例如「先去健身房→館長介紹你認識教練→教練約你特訓」三段連鎖），就要用到旗標。'));

  wrap.append(h(4, '事件選項／結果的三個新欄位'));
  wrap.append(field('點亮事件旗標（setFlags）', '寫一個旗標鍵。下一段事件的條件用 <code>[&#39;eventFlag&#39;, 鍵]</code> 讀它。'));
  wrap.append(field('熄滅事件旗標（clearFlags）', '一段連鎖走完一定要熄。不熄的話那張條件卡會月月霸佔事件一。'));
  wrap.append(field('具名計數（counters）', '跨卡累加的計數。同一個鍵寫在多張卡上，次數會累加。'));

  wrap.append(h(4, '條件式新節點'));
  wrap.append(code(`['eventFlag', '鍵']              旗標亮著才觸發
['not', ['eventFlag', '鍵']]      旗標沒亮才觸發
['eventCount', '鍵', 'gte', 7]    計數 ≥ 7 才觸發`));

  wrap.append(p('<strong>年度閂</strong>：旗標鍵帶 <code>annual_</code> 前綴時，年初會自動清掉——代表「一年一次」而不是「一輩子一次」。'));

  /* ---- 步驟 6 ---- */
  wrap.append(step(6, '輸出與貼回'));
  wrap.append(p('1. 點「複製片段」複製事件卡代碼'));
  wrap.append(p('2. 貼到 <code>src/data/events.js</code> 的 EVENT_CARDS 陣列'));
  wrap.append(p('3. 特質貼到 <code>src/data/epics.js</code> 的 UNIQUE_TRAITS 物件（如果是獨有特質）'));
  wrap.append(p('4. 跑 <code>npm test</code> 確認無錯'));

  /* ---- 速查 ---- */
  wrap.append(h(3, '欄位速查'));

  wrap.append(p('<strong>六屬性</strong>'));
  wrap.append(code(`vit 體能    agi 靈巧    awr 意識
tec 技巧    syn 默契    dec 決斷`));

  wrap.append(p('<strong>六心理維度（mental 欄位）</strong>'));
  wrap.append(code(`comp 抗壓    conf 自信    drive 動機
disc 紀律    trust 信任   resl 韌性`));

  wrap.append(p('<strong>特質階級與取得方式</strong>'));
  wrap.append(code(`common    事件旗標一次觸發
rare      通用特質合成
epic      稀有+通用合成
legend    傳說任務卡達成
unique    生涯條件自動授予（grantWhen）`));

  wrap.append(p('<strong>事件結果的欄位</strong>'));
  wrap.append(code(`text        結果敘事文字
attr        屬性增減（如 vit: 2）
mental      心理維度增減（如 drive: -2）
flags       旗標（觸發 common 特質解鎖）
setFlags    點亮事件旗標（連續事件用）
clearFlags  熄滅事件旗標（連鎖收尾用）
counters    具名計數 +1`));

  wrap.append(p('<strong>效果鍵常見例子</strong>'));
  wrap.append(code(`injuryRate       受傷率封頂
injuryImmune     完全免疫受傷（旗標寫法）
mental_comp      抗壓值增減
mental_drive     動機值增減
mental_disc      紀律值增減
peak_age_shift   巔峰期延後（年）`));

  wrap.append(warn('特質效果鍵只能改機制（機率、倍率、心理值），不能直接改六屬性。屬性增減只能由事件結果（attr 欄位）做。'));
