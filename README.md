...
純文字的 LoL 電競選手生涯養成遊戲。從 2015 年出道打進主場賽區（LMS→PCS→LCP），也能選擇加入海外頂級賽區（LCK／LPL／LEC／LCS），一路打到 MSI 與世界賽。以月為回合管理體力與訓練，賽事期間逐場下決策，退役時生成專屬生涯傳記。
...
引擎完全不依賴 DOM，所以這套測試會在 Node 裡實際跑完 160 段生涯，並驗證體力節奏、訓練事件卡、四階特質合成、條件語言、史實賽制、市場淘汰與退役結局。目前 19912 項檢查全綠。
...
> 規格書 v4.5.3 定案。說「動工」即由 `docs/v4/next-station.mjs` 抓下一站續做。