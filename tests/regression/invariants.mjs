...
function styleGap({ check, log, runs }) {
  ...
  /*
   * ⚠ **門檻 0.9 → 0.85（S46）**：錯位輪替把六屬性的曝光拉成完全均等，屬性權重平坦的
   * 位置（MID／ADC）因此「選哪個活動」的差別變小——實測 87.5%（70/80），落後的十組
   * 全在那兩個位置。平均領先反而從 0.137 升到 0.158（集中投資在潛力衰減下的代價變
   * 明顯），所以這不是打法差距被吃掉，是它從「幾乎每一份天賦都贏一點」變成「贏的時候
   * 贏更多、平坦位置上互有勝負」。門檻跟著新權重重設，不是放寬要求。
   */
  check('打法差距：老手選活動在八成五以上的天賦上都不落後',
    winRate >= 0.85, `勝出 ${(winRate * 100).toFixed(1)}%（${diffs.filter((d) => d > 0).length}/${diffs.length}）`);
