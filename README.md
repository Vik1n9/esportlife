...
引擎完全不依賴 DOM，所以這套測試會在 Node 裡實際跑完 160 段生涯，並驗證種子界線、生涯評價分布、特質合成、版本落差方向、史實賽制、體力經濟與解散名單過濾。目前 18932 項檢查全綠。
...
V4 重建共 52 站（基礎 31 站＋組裝前修復與追加 7 站＋庚組 NPC 選手與戰隊 8 站＋辛組賽事模擬 6 站），已完成 31 站——甲～戊全數收斂，全部可遊玩內容已上線。己組進行中：S20c 交接鏈修復已完成（名次改以機器可讀的鍵入帳，世界賽冠亞軍與賽段冠軍終於在生涯查詢與評分裡算數；獨有特質接線上線），下一站 S20g 賽事成績帳本與衛冕者；庚辛兩組定案已併入規格，施工待前組收斂。詳細狀態表見 `docs/v4/README.md`。
...
| 己 · 組裝（S20c–S22） | 🔧 進行中（S20c 交接鏈修復完成；S20g／S20h／S20d–S20f 待做，再進 DEMO 組裝與文件同步） |
| 庚 · NPC 選手與戰隊（S23–S30） | ⏳ 定案已併（v4.3.3 §23，施工未開始） |
| 辛 · 賽事模擬與戰績參照（S31–S36） | ⏳ 定案已併（v4.4 §24，施工未開始） |

> 規格書已升到 v4.4（v4.3：生命週期曲線、訓練事件卡、特質重建、市場淘汰；v4.3.1：編輯器整合、四池管理、訓練邊界廢除；v4.3.3：庚組 NPC 選手與戰隊 §23；v4.4：辛組賽事模擬與戰績參照 §24）。戊組全數收斂：S18 事件卡 86＋生涯任務卡 25＋訓練卡 60 分批做完、S19b 傳說特質擴到 20、S19c 配方重製（FUSIONS 重寫、死卡全救活）、S21a 生涯傳記上線。庚辛兩組細部規格（NPC schema、百分位母體、模擬引擎）由 S23／S31 站定案後回寫。`npm test` 18932 項全綠，SAVE_VERSION 20（S20c 起舊存檔作廢）。說「動工」即由 `docs/v4/next-station.mjs` 抓下一站續做。