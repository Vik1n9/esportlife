...
- **側欄 sticky 吃 `--board-h`**（S42b）：狀態帶是五列且列4 會折疊，高度不是常數，
  由 `ui/board.js` 量測寫回。左右欄的 sticky 落點與可視高度一律讀它，不寫死像素。
...
- inline 卡牌選項：接在卡片下緣的選項組，無獨立容器邊框，靠 22px 縮排與卡片對齊；
  選定後就地換成定格文字（`--dim`），不留可按外觀。
