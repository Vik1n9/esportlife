...
V4 重建共 31 站（v4.3 新增 S15b 生命週期曲線站），已完成 16 站，全部可遊玩內容皆已上線。詳細狀態表見 `docs/v4/README.md`。
...
| 丁 · 回合與訓練（S14–S15b） | ✅ 完成（月回合制／賽事事件序列）；S15b 生命週期曲線未開始 |
| 戊 · 內容（S17–S21a） | ⏳ 未開始（事件觸發引擎／生涯任務引擎／內容編輯器／特質與配方擴充） |
| 己 · 組裝（S21–S22） | ⏳ 未開始（DEMO 組裝／文件同步） |

> 規格書已升到 v4.3.1（v4.3：生命週期曲線、訓練事件卡、特質重建、市場淘汰；v4.3.1：編輯器整合、四池管理、訓練邊界廢除）。v4.3 新增 **S15b 生命週期曲線站**（統一成長與衰退曲線，30 站變 31 站）。丁組下一站是 S15b；`docs/v4/README.md` 狀態表與各站說明書已對齊 v4.3.1。說「動工」即由 `docs/v4/next-station.mjs` 抓下一站續做。