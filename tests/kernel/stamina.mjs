...
import {
  ...
  REST_RECOVER, STAMINA_MAX, TRAIN_COST, applyMonthAction, bandOf, canRest, consume,
  formFactor, injuryMul, monthActions, monthlyDrift, recover, recoveryOptions, seriesCost,
...
export async function run({ check, log }) {
  ...
  /* ---- §6.3 賽事期間：不能休息，只能減少訓練 ---- */
  {
    ...
    // 月回合的選單走同一條規則：平時四個行動、賽事期間沒有「休息」
    check('養成回合選得到休息', monthActions({}).some((a) => a.id === 'rest'));
    check('賽事期間的月選單沒有休息（§6.3）', !monthActions({ inEvent: true }).some((a) => a.id === 'rest'));
    check('每個行動都標了體力價格與成長級別',
      monthActions({}).every((a) => typeof a.cost === 'number' && 'grow' in a));
    check('只有訓練類的行動會成長',
      monthActions({}).filter((a) => a.grow).map((a) => a.id).join() === 'train,light');
  }

  /* ---- 每月行動：引擎報價，玩家決定（S14 換掉了 S13 的自動駕駛） ---- */
  {
    const tired = fresh('act', { stamina: 20 });
    applyMonthAction(tired, 'rest');
    check('休息回得多', tired.stamina >= 20 + REST_RECOVER * 0.8, tired.stamina);

    const worn = fresh('act2', { stamina: 60, tempInjuryRisk: 10 });
    const before = worn.stamina;
    applyMonthAction(worn, 'rehab');
    check('復健預防回得比休息少', worn.stamina - before < REST_RECOVER, worn.stamina - before);
    check('復健預防壓得下臨時受傷風險（§5.2 它唯一的賣點）',
      worn.tempInjuryRisk < 10, worn.tempInjuryRisk);
    check('復健預防的恢復量對得上 §6.1', Math.abs((worn.stamina - before) - REHAB_RECOVER) <= REHAB_RECOVER * 0.25);

    const grind = fresh('act3');
    applyMonthAction(grind, 'train');
    check('全力訓練扣得最重', STAMINA_MAX - grind.stamina === TRAIN_COST, grind.stamina);

    // 保守玩法的判準本身（策略住在測試網，不在引擎裡）
    const menu = { options: monthActions({}) };
    check('保守玩法：體力夠就練', monthAction(fresh('p1', { stamina: REST_AT + 1 }), menu) === 'train');
    check('保守玩法：體力不夠就休息', monthAction(fresh('p2', { stamina: REST_AT - 1 }), menu) === 'rest');
    check('保守玩法：賽事期間不能休息時改減量訓練',
      monthAction(fresh('p3', { stamina: 10 }), { options: monthActions({ inEvent: true }) }) === 'light');
  }

  /* ---- 經濟：一次休息換得回幾個訓練月 ---- */
  {
    /*
     * §6.1 的四個數字合起來要撐得住 3–4 個養成回合的節奏，這是它的算術骨架。
     *
    ...
    const drainPerTrainMonth = TRAIN_COST + matchCost - MONTHLY_RECOVER;
    ...
    check('賽事月不是白給的：一年的自然恢復擋不掉一年的訓練消耗',
      CAL_MONTHS * MONTHLY_RECOVER < dev.length * TRAIN_COST,
      `${CAL_MONTHS * MONTHLY_RECOVER} vs ${(dev.length * TRAIN_COST).toFixed(0)}`);
