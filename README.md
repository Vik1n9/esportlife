...
V4 重建共 53 站（基礎 31 站＋組裝前修復與追加 8 站＋庚組 NPC 選手與戰隊 8 站＋辛組賽事模擬 6 站），已完成 32 站——甲～戊全數收斂，全部可遊玩內容已上線。己組進行中：S20c 交接鏈修復完成、S20g 賽事成績帳本與衛冕者完成（世界賽冠軍登記表 `titleHistory` 上線，衛冕者身分與 `torch_bearer` 可取得，事件卡條件改吃 `evalCond` 統一條件語言），下一站 S20h 卡片文本變數；庚辛兩組定案已併入規格，施工待前組收斂。詳細狀態表見 `docs/v4/README.md`。
