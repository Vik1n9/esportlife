 ...
 *     純軌跡不吃素材全都合法。本表刻意多樣化——12 張吃兩個素材、8 張純戰績／
 *     軌跡（worlds-king／grand_slam／goat／rookie-king 戰績型）。§14.2 的
 *     「每史詩恰好進兩條、稀有依取得率分配」只在「每卡一史詩一稀有」時成立，
 *     本表稀有只用 5 次（immortal／godslayer／record_breaker／mr_clutch），
 *     其餘素材全用通用——通用共用是合法的，S19c 重推供給算式時以實際卡表為準。
 *   - ⚠ 素材一律選 160 段實測「生涯尾持有率 ≥10%」的特質（2026-08-15 量測，
 *     見交接筆記附表）。死路檢驗（§14.2）的死線是 15%，邊緣素材
 *     （genius 11%／intlghost 10%／underdog 14%／disc 14%）由 S19c 校準——
 *     它們都有取得途徑（事件卡／扮演卡），只是被合成樹消耗，持有窗口窄。
 *   - 達成目標全部以「160 段實測可達」為草案底線（世界賽決賽 160 段 0 段、
 *     splitTitles ≥4 幾乎不可達、assistsPerGame max 3.0——交接筆記有分布表）。
 *     goal 必須比 trigger 門檻高，否則開卡當下瞬間達成。
 *   - 素材在**達成時**才消耗（§14.1）；期限內失敗素材保留，可以走別條路。
 *   - deadline 以賽季計（null = 生涯結束前），數值全是草案，S19c 實測校準。
 *
...
export const QUEST_CARDS = [
  ...
  },
  {
    ...
    trigger: ['and', ['has', 'epic', 'soloking'], ['has', 'rare', 'traffic']],
    goal: ['stat', 'worldsBest', 'lte', 3],
    deadline: 2,
    result: { tier: 'legendary', key: 'godslayer' },
    failLabel: '弒神未竟',
    goalText: '達成目標：站上世界賽四強（含以上）',
    materialText: '所需素材：賽區之光（史詩）＋ 流量密碼（稀有）',
  },
  {
    id: 'legend-bulwark',
    type: 'legend',
    ...
    goal: ['and', ['stat', 'splitTitles', 'gte', 2], ['stat', 'intlWinRate', 'gte', 40]],
    deadline: 2,
  ...
  },
  {
    ...
    trigger: ['and', ['has', 'rare', 'star'], ['has', 'common', 'popular']],
    ...
    materialText: '所需素材：明星選手（稀有）＋ 人氣選手（通用）',
  },
  {
    ...
    trigger: ['and', ['has', 'common', 'underdog'], ['has', 'common', 'glue']],
    goal: ['and', ['stat', 'worldsBest', 'lte', 3], ['stat', 'msiBest', 'lte', 3]],
    deadline: 2,
    result: { tier: 'legendary', key: 'underdog_run' },
    failLabel: '止步的黑馬',
    goalText: '達成目標：世界賽與 MSI 都站上過四強',
    materialText: '所需素材：逆風翻盤（通用）＋ 休息室黏著劑（通用）',
  },
  {
    ...
    goal: ['and', ['stat', 'splitTitles', 'gte', 2], ['stat', 'awards', 'gte', 4]],
    deadline: 2,
    result: { tier: 'legendary', key: 'prophet_king' },
    failLabel: '過期的版本答案',
    goalText: '達成目標：生涯累計賽段冠軍 ≥ 2，且生涯獎項 ≥ 4',
  ...
  },
  {
    ...
    goal: ['and', ['stat', 'awards', 'gte', 4], ['stat', 'splitTitles', 'gte', 1]],
    deadline: 2,
    result: { tier: 'legendary', key: 'heavenly' },
    failLabel: '墜落的天神',
    goalText: '達成目標：生涯獎項 ≥ 4，且生涯累計賽段冠軍 ≥ 1',
  ...
  },
  {
    ...
    goal: ['and', ['stat', 'splitTitles', 'gte', 2], ['stat', 'intlWinRate', 'gte', 45]],
    deadline: 2,
    result: { tier: 'legendary', key: 'shotcaller' },
    failLabel: '燈滅的指揮官',
    goalText: '達成目標：生涯累計賽段冠軍 ≥ 2，且國際賽勝率 ≥ 45%',
  ...
  },
  {
    ...
    goal: ['and', ['stat', 'splitTitles', 'gte', 2], ['stat', 'awards', 'gte', 2]],
    deadline: 2,
  ...
  },
  {
    ...
    trigger: ['and', ['has', 'common', 'trashtalk'], ['has', 'common', 'grinder']],
    goal: ['and', ['stat', 'awards', 'gte', 5], ['stat', 'splitTitles', 'gte', 1]],
    deadline: 2,
    result: { tier: 'legendary', key: 'one_man_army' },
    failLabel: '一人的孤軍',
    goalText: '達成目標：生涯獎項 ≥ 5，且生涯累計賽段冠軍 ≥ 1',
    materialText: '所需素材：嘴砲王（通用）＋ 肝帝（通用）',
  },
  {
    id: 'legend-iron-king',
    type: 'legend',
  ...
  },
  {
    ...
    goal: ['and', ['stat', 'intlWinRate', 'gte', 45], ['stat', 'splitTitles', 'gte', 1]],
    deadline: 2,
    result: { tier: 'legendary', key: 'late_game' },
    failLabel: '破功的後期',
    goalText: '達成目標：國際賽勝率 ≥ 45%，且生涯累計賽段冠軍 ≥ 1',
  ...
  },
  {
    ...
    trigger: ['and', ['has', 'common', 'camera'], ['has', 'rare', 'star']],
    goal: ['and', ['stat', 'msiBest', 'lte', 2], ['stat', 'awards', 'gte', 3]],
    deadline: 2,
    result: { tier: 'legendary', key: 'adc_king' },
    failLabel: '被切死的輸出',
    goalText: '達成目標：站上 MSI 決賽舞台（亞軍以上），且生涯獎項 ≥ 3',
    materialText: '所需素材：鏡頭感（通用）＋ 明星選手（稀有）',
  },
  {
    id: 'legend-mr-clutch',
    type: 'legend',
    ...
    trigger: ['and', ['has', 'common', 'intlghost'], ['has', 'rare', 'star']],
    goal: ['and', ['stat', 'msiBest', 'lte', 2], ['stat', 'splitTitles', 'gte', 1]],
    deadline: 2,
    result: { tier: 'legendary', key: 'mr_clutch' },
    failLabel: '熄火的關鍵局',
    goalText: '達成目標：站上 MSI 決賽舞台（亞軍以上），且生涯累計賽段冠軍 ≥ 1',
    materialText: '所需素材：國際賽之鬼（通用）＋ 明星選手（稀有）',
  },
  {
...

  {
    ...
    trigger: ['and', ['stat', 'worldsBest', 'lte', 4], ['stat', 'awards', 'gte', 3]],
    goal: ['and', ['stat', 'worldsBest', 'lte', 3], ['stat', 'awards', 'gte', 5]],
    deadline: null,
    result: { tier: 'legendary', key: 'worlds_king' },
    failLabel: '決賽的常客',
    goalText: '達成目標：站上世界賽四強，且生涯獎項 ≥ 5',
    materialText: null,
  },
  {
    ...
    goal: ['and', ['stat', 'msiBest', 'lte', 1], ['stat', 'worldsBest', 'lte', 3]],
    deadline: null,
    result: { tier: 'legendary', key: 'grand_slam' },
    failLabel: '差一座的滿貫',
    goalText: '達成目標：MSI 奪冠，且世界賽站上過四強',
    materialText: null,
  },
  {
    id: 'legend-goat',
    ...
    goal: ['and', ['stat', 'splitTitles', 'gte', 4], ['stat', 'awards', 'gte', 10]],
    deadline: null,
    result: { tier: 'legendary', key: 'goat' },
    failLabel: '歷史的注腳',
    goalText: '達成目標：生涯累計賽段冠軍 ≥ 4，且生涯獎項 ≥ 10',
    materialText: null,
  },
  {
    ...
    trigger: ['and', ['stat', 'proYears', 'lte', 3], ['stat', 'splitTitles', 'gte', 1]],
    goal: ['and', ['stat', 'proYears', 'lte', 5], ['stat', 'msiBest', 'lte', 3]],
    deadline: 2,
  ...
  },
  {
    ...
    goal: ['and', ['stat', 'distinctTeams', 'gte', 5], ['stat', 'coachRating', 'gte', 60]],
    deadline: 3,
    result: { tier: 'epic', key: 'ascetic', label: '流浪傭兵' },
    failLabel: '找不到家的浪人',
    goalText: '達成目標：生涯效力 ≥ 5 支不同戰隊，且教練評價 ≥ 60',
    materialText: null,
  },
  {
    id: 'route-team-soul',
    ...
    goal: ['stat', 'longestTenure', 'gte', 9],
    deadline: null,
    result: { tier: 'epic', key: 'lockerroom', label: '不滅隊魂' },
    failLabel: '離隊的元老',
    goalText: '達成目標：效力同一戰隊 ≥ 9 年',
    materialText: null,
  },
  {
    id: 'route-green-leaf',
